...
export default {
 manifest: {
  ...
  apiVersion: 1,
  version: "1.3.1",
  description: "相机按钮展开工具小窗，保留原发送、表情、贴纸、加号和 AI 回复功能。需搭配灰白版 CSS。",
 ...
 setup(ctx) {
  ...
  ctx.ui.slot('settings.section', el => {
    const render = () => {
      el.replaceChildren();
      el.style.cssText='padding:14px;background:#e6e6e6;border:1px solid #b6b6b6;border-radius:8px;color:#7e7e7e';
      ...
      for(const [mode,label] of [['all','全部开启'],['off','全部关闭'],['selected','仅开启已勾选']]) {
        const b=document.createElement('button');b.textContent=label;b.type='button';b.style.cssText='margin:4px;padding:8px;border-radius:5px;background:#8c8c8c;color:white';
